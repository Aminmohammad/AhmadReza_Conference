...
function [ranked_all_Chromosomes_of_all_Iterations all_Chromosomes_of_all_Iterations Bests_of_all_Iterations Global_Best] = Genetic_Algorithm_Manager (     initial_Population, ...                  % A Matrix or '' :  Each Column has the results of a Test for All Variables      
                                                                                                                                                            ...
                                                                                                                                                            temp_Results_Folder_Address )                           % The Address of Where data Will be Saved.                                                                                     %             : for 'Ascend'  the 'Global Best' 'descreases'
...
%             end
            ...
            index_of_Slash = strfind( temp_Results_Folder_Address, '/');
            if ( isempty ( index_of_Slash ) == 0 )
                current_Saving_Address = sprintf ( '%s//Chromosomoe_Population_%d.m', temp_Results_Folder_Address, 1 );

            else
                current_Saving_Address = sprintf ( '%s\\Chromosomoe_Population_%d.m', temp_Results_Folder_Address, 1 );

            end
            save ( current_Saving_Address, 'chromosome_Population' );            
...
%                 mutated_Populations = repmat(mutated_Populations, n_Mutation, 1);
            ...
            %% Part 3 : Acumulation of Chromosome_Population && Crossovered_parents && Mutated_Population
                ...
                % Saving All Chromosomes from all Iterations
                    total_Chromosomes_of_each_Iteration = [crossovered_parents; mutated_Populations]; %#ok<*NASGU>
%                     priliminary_Size_of_All_Chromosomes_of_all_Iterations = size( all_Chromosomes_of_all_Iterations, 1);
                    
                    index_of_Slash = strfind( temp_Results_Folder_Address, '/');
                    if ( isempty ( index_of_Slash ) == 0 )
                        current_Saving_Address = sprintf ( '%s//Chromosomoe_Population_%d.m', temp_Results_Folder_Address, iteration_Index + 1 );
                        
                    else
                        current_Saving_Address = sprintf ( '%s\\Chromosomoe_Population_%d.m', temp_Results_Folder_Address, iteration_Index + 1 );
                        
                    end
                    save ( current_Saving_Address, 'total_Chromosomes_of_each_Iteration' );
    ...
    %% phase 5 : Returning the Output
        ...
        % Making the 'all_Chromosomes_of_all_Iterations' Structure
            % Loading the Data From Hard Drive
                temp_Results_Folder_Content = dir ( temp_Results_Folder_Address );
                number_of_Items_in_Temp_Results_Folder = size ( temp_Results_Folder_Content, 1 );
                
                
%                 for content_Index = 1 : number_of_Items_in_Temp_Results_Folder
%                     address_of_Current_Item = sprintf ()
%                 end
...
%             all_Chromosomes_of_all_Iterations = all_Chromosomes_of_all_Iterations ( population_Size + 1 : end , 1 ) ; % Eliminate this line  : Just for  Mr. Behboodi'd Project
            ...
            for index = 1 : size ( all_Chromosomes_of_all_Iterations, 1 )
                ranked_all_Chromosomes_of_all_Iterations ( index, 1). value = [all_Chromosomes_of_all_Iterations(Index_Vector( 1, index)).value];
                ranked_all_Chromosomes_of_all_Iterations ( index, 1). Total_Cost = [all_Chromosomes_of_all_Iterations(Index_Vector( 1, index)).Total_Cost];
            end
    ...
    %% phase 6 : Attaching ( Lambda-Populations && CodeRate && SimgaLambda && F && FINT ) Together  
        ...
        for index = 1 : Iteration_UpperBound
            ...
            temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( index , number_of_Vars + 1 ) = Code_Rate_Calculator ( temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( index , 1 : 7 ), initial_Parameters ( 1, 2 ) );
    ...
    %% phase 10 : Drawing BestFittness
        if (draw_the_Convergence_Fig == 1)
            ...
            % ranked all Chromosomes of all Iterations ( all CodeRates of all Iterations )                 
                ...
                plot( x_Label, temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( :, 8 ), '*r', 'DisplayName', 'Code-Rate' );
            ...
            % ranked all Chromosomes of all Iterations ( all SigmaLambdas of all Iterations ) 
                hold all;
                plot( x_Label, temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( :, 9 ), '--b', 'DisplayName','Total Cost (  SigmaLambda )');
                
            % ranked all Chromosomes of all Iterations ( all Fs of all Iterations ) 
                hold all;
                plot( x_Label, temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( :, 10 ), '-.g', 'DisplayName','F'); 
                
            % ranked all Chromosomes of all Iterations ( all FINTs of all Iterations ) 
                hold all;
                plot( x_Label, temp_Matrix_of_all_Chromosomes_of_all_Iterations_and_CodeRate_and_LambdaSigma_and_F_and_FINT ( :, 11 ), 'y', 'DisplayName','FInt');
